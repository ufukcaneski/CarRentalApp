...
const AdminDashboard = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    let result = await addCar(name, url, rentInWei, saleInWei);
    if (result) {
  ...
  const editCarStatusData = async (e) => {
    e.preventDefault();
    try {
      let result = await editCarStatus(id, status);
      if (result) {
  ...
  return (
    <div className="bg-gradient-to-r  border-t-2  border-white  from-slate-700 via-pink-900 to-black">
      <div className="flex justify-center gap-x-10 items-center">
        ...
        <form className="" onSubmit={editCarData}>
          <div>
            <label className="text-white" htmlFor="name">
              Id:
            </label>
          ...
          <div>
            ...
            <input
              type="text"
              placeholder="New Car Picture Url"
        ...
        <form onSubmit={editCarStatusData}>
          <div>
...
// he can add new car and edit car metadata and status and also he can withdraw balance from contract and set new owner for contract